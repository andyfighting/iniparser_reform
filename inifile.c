...
void iniparser_dump_ini(const dictionary * d, FILE * f)
{
    ...
    if (nsec<1) {
        /* No section in file: dump all keys as they are  | modified by Andy*/
        /*added by Andy 20150601: if section eq NULL, then add new section.*/
        if (d->key[0]!=NULL) {
            sscanf(d->key[0], "%[^:]", new_sec);
            fprintf(f, "[%s]\n", new_sec);
            if((str_ptr = strchr(d->key[0], ':')))
                fprintf(f, "%s=%s\n", (str_ptr + 1), d->val[0]);
        }
    }
...
int read_profile_string( const char *section, const char *key,char *value, int size,const char *default_value, const char *file)
{
    ...
    snprintf(str_key, DICTMINSZ, "%s:%s", section, key);
    s = iniparser_getstring(ini, str_key, NULL);

    if(!s && (default_value != NULL)) {
        strncpy(value,default_value, size);
    }
    else{
        memset(value, 0, size);
		snprintf(value, size, "%s", s);
    }
...
{
    dictionary  *ini ;
    char  sec_key[DICTMINSZ];
    FILE *out;

    if (strlen(value)<1) {
        fprintf(stderr, "%s value is not usable\n", file);
        return 1 ;
    }

    snprintf(sec_key, DICTMINSZ, "%s:%s", section, key);
